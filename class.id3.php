...
class id3v2 {
    ...
    */
    ...
    // XXX: For now we'll assume ID3v2 tags are only at the beggining of the 
file.
    $r = fread($f, 10);
...
fingers</B>\n";
    ...
    // The 2.3 and 2.4 frames have the same frame header format but the bits 
aren't the same.
...
"</B>\n");
        ...
        if ($header['FLAGS']['unsynchronisation'] or 
$fheader['unsynchronisation']) {
...
'a3TAG/a30NAME/a30ARTISTS/a30ALBUM/a4YEAR/a30COMMENT/C1GENRENO';
    ...
    } while (!$bits[8] and !$bits[9] and !$bits[10]); // 1st 8 bits true from 
the while
